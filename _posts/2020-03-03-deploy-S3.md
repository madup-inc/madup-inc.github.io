---
title: S3 정적 사이트 배포
excerpt: S3, CloudFront를 사용하여 SPA를 호스팅하는 글을 번역하였습니다.
image: thumbnail/deploy-S3.jpg
categories: [tech]
author: jason
---

> 아래 글은 [출처](https://gist.github.com/bradwestfall/b5b0e450015dbc9b4e56e5f398df48ff){:target="_blank"}의 글을 [저자](https://gist.github.com/bradwestfall){:target="_blank"}의 허락을 받아 번역한 내용입니다.
...
## SPA
웹 사이트가 SPA인 경우 파일이 없는 경우에도 서버 (이 경우 S3)에 대한 모든 요청이 무언가를 반환하는지 확인해야합니다.
...
## 배포
배포를 위해 CloudFront CDN의 파일이 변경되지 않았음을 고려해야합니다.
새 파일을 S3에 업로드 할 경우 CDN의 `Edge` 서버에 배포되지 않으므로 웹 사이트가 업데이트되지 않습니다.
[자세히 읽기](http://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/ReplacingObjectsSameName.html){:target="_blank"}.

CDN에서 파일을 무효화하려면 CloudFront의 **Invalidations** 기능을 사용해야합니다.
[자세히 읽기](http://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/Invalidation.html){:target="_blank"}.
...
무효화와 함께 배포하려면 먼저 [AWS-CLI](https://aws.amazon.com/cli){:target="_blank"}를 설치해야합니다.
