...
전편이 실패담이었다면 이번에는 어떻게 적용했는지가 주요 내용입니다. 
---
...
계속 격어온 AWS Elastic Beanstalk과 연동하는 문제 역시 Travis에서 지원해주는 배포툴 중 하나이기에 딱 배포하는 것 까지는 문제가 없었습니다. 
...
몇 가지 햇갈렸던 옵션들에 대해서만 주석으로 기제합니다.

이렇게 하면 일단 배포는 되지만 문제가 생겼습니다. S3에 들어가보면 앱의 이름이 이상하게 되어있는걸 발견했습니다.
...
슬랙연동은 해줄일이 정말 없습니다. 슬랙에서 APP을 추가해서 토큰을 받아온후 그 토큰을 yml에 기제하면 끝입니다.
...
사실 아직도 해결이 안된 부분이고 가장 힘든 부분입니다.
revenue 백앤드 코드의 대부분은 DB 작업인데 (가장 복잡한고 최근에 리팩토링한 부분도 DB 작업을 대신해주는 인터페이스)
DB는 몇몇 EC2만 접속이 가능해서 SSH 터널링을해야 API 테스트가 가능했습니다.

이 과정에서 왜 문제가 일어나는지도 몰랐고 해결방법도 모르고 결국 알아냈지만 또 문제가 생겨서 쉘만 잔득 만들었습니다.

대략적으로 격었던 과정은 다음과 같습니다.

1. db 접속확인을 위해 테스트후에 결과를 저장하고 DB 커넥션 관련 에러만 보여주는 쉘을 작성
...
1. S3에 파일이 war가 아니라 zip으로 들어가게 찝찝함
...
### 현기증나니깐 뭐 하나 정해써요
두 가지 배포자동화를 해놯서 매번 푸쉬할 때 마다 두 중 하나를 수동으로 정지시키는 고생을 하다가
...
그 다음으로 드는 생각은 제이슨에게 감사함을 느낌니다. 제이슨이 왠만한 교수보다 잘가르친다고 느낄 정도로 제이슨이 실전 내공과 리눅스 기초 지식을
알려주셔서 많은 공부가 되었습니다. 제 친구들도 저한테 이렇게 잘알려주지 않았는데 제이슨이 정말 많이 알려주셨습니다.
